...
import { calculateItemPrice, round2 } from '../carts/utils/cart-price.utils';
...
export class CheckoutService {
    ...
    async createCartCheckoutIntent(
        ...
        // 2️⃣ Fetch banners and calculate prices for each item
        const itemsWithPrices = await Promise.all(
            selectedItems.map(async (item: any) => {
                ...
                // Add banner to each addon (NO pricing touched here)
                const addonsWithBanners = (item.addons || []).map((addon: any) => ({
                    ...addon,
                    banner: addonBannerMap.get(addon.addonId?.toString()) || [],
                }));

                // ✅ Authoritative pricing (tiersWithSlot + quantity + discounts)
                const priceBreakdown = calculateItemPrice({
                    ...item,
                    addons: addonsWithBanners,
                });
                console.log("price breakdown amount", priceBreakdown)
                return {
                    ...item,
                    banner: eventBanner,
                    addons: addonsWithBanners,
                    ...
                    // Price snapshot
                    originalBasePrice: priceBreakdown.originalBasePrice,
                    discountPercent: priceBreakdown.discountPercent,
                    discountAmount: priceBreakdown.discountAmount,
                    finalBasePrice: priceBreakdown.finalBasePrice,
                    addonTotal: priceBreakdown.addonTotal,
                    plannerPrice: priceBreakdown.plannerPrice,
                    subtotal: priceBreakdown.subtotal,
                };
            }),
        );

        // 3️⃣ Calculate subtotal from all items (already rounded per item)
        const subtotal = round2(
            itemsWithPrices.reduce((sum, item) => sum + (item.subtotal || 0), 0),
        );
        ...
        // ✅ Always round after coupon math
    ...
    async createDirectCheckoutIntent(
        ...
        if (!draft.eventDate || draft.eventCategory !== 'AddOn' && !draft.eventTime) {
        ...
        // 7️⃣ Calculate price breakdown using utility
        const priceBreakdown = calculateItemPrice(checkoutItem);
        ...
        // if (couponCode) {
        //   const result = await this.applyCoupon(couponCode, priceBreakdown.subtotal);
        //   discount = result.discount;
        //   couponSnapshot = result.snapshot;
        // }

        const totalAmount = round2(priceBreakdown.subtotal - discount);

        // 9️⃣ Create Checkout Intent
        const checkoutIntent = await this.checkoutIntentModel.create({
            userId,
            source: 'direct',
            items: [
    ...
    async getCheckoutIntentWithDetails(
        ...
        // make sure result exists and is array
        if (!Array.isArray(result) || !result.length) {
            throw new BadRequestException('No checkout result found');
        }

        const items = result[0].items || [];

        const eventDates = items
            .map((i: any) => new Date(i.eventBookingDate))
        ...
        const nearestEventDate = eventDates.reduce((a: Date, b: Date) =>
            a < b ? a : b
        );

        ;

        result[0].nearestEventDate = nearestEventDate;
