...
export class BirthdayEventService {
  ...
  async findAll(options: any = {}) {
    ...
    if (eventDate && events.length) {
      ...
      const bookings =
        await this.orderAvailabilityService.getBookingCountsForEvents({
          eventIds,
          city,
  ...
  async update(
    ...
    const updateData: Partial<BirthdayEventDocument> = Object.fromEntries(
      Object.entries(dto).filter(
        ([key, val]) => updatableFields.includes(key) && val !== undefined,
      ),
    );
      ...
      await Promise.allSettled(
        deletedBanners.map(async (bannerUrl) => {
          ...
          try {
            ...
            console.error(
              `❌ Failed to delete S3 image: ${bannerUrl}`,
              err.message,
            );
          }
        }),
    ...
    if (effectiveAgeGroup === 'kids' && !effectiveSubCategory) {
      throw new BadRequestException(
        'Sub category is required for kids age group',
      );
    }
  ...
  async deleteByAdmin(id: Types.ObjectId): Promise<BirthdayEventDocument> {
    ...
    if (!event) {
      throw new NotFoundException(
        'Birthday event not found or already deleted',
      );
    }
