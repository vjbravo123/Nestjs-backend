...
export class VendorService {
  ...
  async createVendorByAdmin(
    dto: CreateVendorByAdminDto,
    adminId: string,
    ...
    try {
      ...
      const vendorDocs = await this.vendorModel.create(
        [
          {
            ...dto,
            password: hashedPassword,
      ...
      if (!vendor) throw new InternalServerErrorException('Failed to create vendor');

      /* --------------------------------------------------
         4. AUTH LINK (SAME AS verifyMobileOtp)
      ---------------------------------------------------*/
      ...
      let auth = await this.authService.linkAuthRecord({
        mobile: Number(dto.mobile),
        role: rolesToAdd[0] || 'vendor',
        userId: user?._id ? (user._id as Types.ObjectId) : undefined,
        vendorId: vendor?._id ? (vendor._id as Types.ObjectId) : undefined,
        currentRole: 'vendor',
      });

      // Ensure all roles are added (EXACT OTP LOGIC)
      ...
      /* --------------------------------------------------
         5. CREATE DEFAULT VENDOR AVAILABILITY (all days, all slots)
      ---------------------------------------------------*/
      await this.vendorAvailabilityService.setWeeklyAvailability(
        vendor._id as Types.ObjectId,
        {
          weeklyAvailableDays: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
          weeklySlots: [
            { day: 'Sun', slots: ['breakfast', 'lunch', 'tea', 'dinner'] },
            { day: 'Mon', slots: ['breakfast', 'lunch', 'tea', 'dinner'] },
            { day: 'Tue', slots: ['breakfast', 'lunch', 'tea', 'dinner'] },
            { day: 'Wed', slots: ['breakfast', 'lunch', 'tea', 'dinner'] },
            { day: 'Thu', slots: ['breakfast', 'lunch', 'tea', 'dinner'] },
            { day: 'Fri', slots: ['breakfast', 'lunch', 'tea', 'dinner'] },
            { day: 'Sat', slots: ['breakfast', 'lunch', 'tea', 'dinner'] },
          ],
        },
      );
  ...
  async getVendorWithPendingChanges(vendorId: string) {
    ...
    const pipeline: PipelineStage[] = [
      ...
      /* --------------------------------------------------
       * 2️⃣ Convert pendingChanges.businessType (string → ObjectId)
       * -------------------------------------------------- */
      {
        $addFields: {
          'pendingChanges.businessType': {
            $cond: {
              if: { $ne: ['$pendingChanges.businessType', null] },
              then: { $toObjectId: '$pendingChanges.businessType' },
              else: null,
            },
          },
        },
...

      {
        $addFields: {
          businessType: {
            $cond: {
              if: { $ne: ['$businessType', null] },
              then: { $toObjectId: '$businessType' },
              else: null,
            },
          },
        },
      ...
      /* --------------------------------------------------
       * 4️⃣ Approved businessType
       * -------------------------------------------------- */
      ...lookupAndUnwind(
        'businessType',
        'dropdownoptions',
        'businessType',
        { label: 1, type: 1, value: 1 },
        true, // single object
      ),

      /* --------------------------------------------------
       * 5️⃣ Pending businessType
       * -------------------------------------------------- */
      ...lookupAndUnwind(
        'pendingChanges.businessType',
        'dropdownoptions',
        'pendingBusinessType', // use temp field (important!)
        { label: 1, type: 1, value: 1 },
        false,
      ),
      {
        $addFields: {
          'pendingChanges.businessType': {
            $arrayElemAt: ['$pendingBusinessType', 0],
          },
        },
      },

      /* --------------------------------------------------
       * 6️⃣ Pending servicesOffered (string[] → ObjectId[])
       * -------------------------------------------------- */
      {
        $addFields: {
          pendingServiceIds: {
        ...
        $addFields: {
          'pendingChanges.servicesOffered': {
            $cond: [
              {
                $and: [
                  { $ne: ['$pendingServices', null] },
                  { $isArray: '$pendingServices' },
                  { $gt: [{ $size: '$pendingServices' }, 0] },
                ],
              },
              '$pendingServices',
              '$$REMOVE',
  ...
  async getVendorProfileForAdmin(userId: string, vendorId: string) {
    ...
    const pipeline = [
      ...
      /* --------------------------------------------------
       * 2️⃣ Convert businessType (string → ObjectId)
       * -------------------------------------------------- */
      {
        $addFields: {
          businessType: {
      ...
      /* --------------------------------------------------
       * 3️⃣ Lookup APPROVED businessType
       * -------------------------------------------------- */
      ...lookupAndUnwind(
        'businessType',
        'dropdownoptions',
      ...
      /* --------------------------------------------------
       * 4️⃣ Lookup PENDING businessType
       * -------------------------------------------------- */
      ...lookupAndUnwind(
        'pendingChanges.businessType',
        'dropdownoptions',
      ...
      /* --------------------------------------------------
       * 5️⃣ Lookup APPROVED servicesOffered (EXISTING LOGIC - UNCHANGED)
       * -------------------------------------------------- */
      ...lookupAndUnwind(
        'servicesOffered',
        'categories',
      ...
      /* --------------------------------------------------
       * 6️⃣ Lookup PENDING servicesOffered (EXISTING LOGIC - UNCHANGED)
       * -------------------------------------------------- */
      {
        $lookup: {
          from: 'categories',
      ...
      /* --------------------------------------------------
       * 7️⃣ Normalize Empty Arrays (UNCHANGED)
       * -------------------------------------------------- */
      {
        $addFields: {
          servicesOffered: {
      ...
      /* --------------------------------------------------
       * 8️⃣ Cleanup (UPDATED only temp field removed)
       * -------------------------------------------------- */
      {
        $project: {
          pendingBusinessType: 0,
