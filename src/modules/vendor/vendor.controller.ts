...
export class VendorController {
    ...
    async createVendorByAdmin(
        ...
        if (files) {
            ...
            if (!Array.isArray(displayImagesArray)) {
                throw new NotFoundException('Images not in array');
            }
        }
            ...
            const imageUrls = await Promise.all(
                files.displayImages.map(async (file, index) => {
                    return uploadImageToS3({
                        fileBuffer: file.buffer,
                        key: `vendor-images/admin-${Date.now()}-${index}-${file.originalname}`,
                        contentType: file.mimetype,
                    });
                }),
            );

            dto.displayImages = imageUrls;
        }

        /* -------------------------------- CREATE VENDOR -------------------------------- */

        const vendor = await this.vendorService.createVendorByAdmin(
            dto,
            req.user.id,
        );
