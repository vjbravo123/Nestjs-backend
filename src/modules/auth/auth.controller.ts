...
export class AuthController {
  ...
  // -------------------- Admin Login As (Impersonate User/Vendor) --------------------
  @Post('admin/login-as')
  @HttpCode(200)
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('admin')
  @ApiResponse({ status: 200, description: 'Successfully logged in as target user/vendor' })
  async adminLoginAs(
    @Body() dto: AdminLoginAsDto,
    @Req() req: Request,
    @Res({ passthrough: true }) res: Response,
  ) {
    const { accessToken, refreshToken, role, publicData } = await this.authService.adminLoginAs(dto, req);
