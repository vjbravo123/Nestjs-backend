import { Controller, Post, Body, Delete, UseGuards, Req, NotFoundException, UploadedFiles, UseInterceptors, BadRequestException, ForbiddenException, Get, Query, Patch, Param, Put } from '@nestjs/common';
...
export class ExperientialEventController {
    ...
    async findAllByAdmin(
        @Query() query: AdminQueryExperientialEventDto,
    ) {
        const data =
            await this.experientialEventService.getExperientialEventListByAdmin(query);

        return {
            message: 'Experiential events fetched successfully.',
            data,
        };
    }
    ...
    @Roles('admin')
    async deleteByAdmin(
        @Param('eventId', MongoIdPipe) eventId: Types.ObjectId,
        ...
        const deletedEvent = await this.experientialEventService.deleteByAdmin(eventId);

        return {
            message: 'Experiential event deleted successfully.',
            data: deletedEvent,
        };
    }
}