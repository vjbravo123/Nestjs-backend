...
import {
  AdminApprovalDto,
  AdminQueryAddOnDto,
  RemoveBannerAddOnDto,
  UpdatePendingAddOnDto,
} from './dto/admin-addons.dto'; // 🔹 create this DTO
...
export class AddOnController {
  ...
  @Roles('admin', 'vendor')
  @UseInterceptors(FileFieldsInterceptor([{ name: 'banner', maxCount: 15 }]))
  async create(
  ...
  async getFilteredAddOnCategories(@Query() query: PublicQueryAddOnDto) {
    return this.addOnService.getFilteredAddOnCategories(query);
  }
  ...
  @Roles('vendor')
  @RequireOwnership('vendorId')
  async getAddOnForEdit(@Param('addOnId') addOnId: string, @Req() req: any) {
  ...
  async approveAddOn(
    ...
    if (dto.action === 'approved') {
      ...
      return this.addOnService.rejectAddOn(
        id,
        dto.reason || 'AddOn rejected',
        req.user.vendorId,
      );
    }
  }
  ...
  // ────────────────────────────────────────────────────────────────
  // 🧩 ADMIN: Soft Delete Add-On
  // ────────────────────────────────────────────────────────────────
  @Patch(':addOnId/delete')
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('admin')
  async deleteByAdmin(
    ...
    const deletedAddOn = await this.addOnService.deleteByAdmin(addOnId);
