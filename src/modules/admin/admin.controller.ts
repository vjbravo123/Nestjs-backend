...
export class AdminController {
  ...
  async login(
    ...
    const { admin, accessToken, refreshToken, jti } =
      await this.adminService.validateAdminLogin(
        dto,
        deviceInfo,
        ip,
        userAgent,
  ...
  async refresh(
    ...
    const {
      ...
      adminId,
      userAgent,
    } = await this.adminService.refreshAccessToken(
      refreshToken,
      deviceInfo,
      ip,
      deviceInfo,
  ...
  async getMigrationsStatus() {
    try {
      ...
      const migrationsCollection = this.connection.db.collection('changelog');
      ...
      return {
        ...
        status:
          migrations.length > 0 ? 'migrations_applied' : 'no_migrations_found',
      };
    } catch (error) {
      return {
