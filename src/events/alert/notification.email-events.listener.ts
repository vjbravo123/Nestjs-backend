...
export class NotificationEmailEventsListener {
    ...
    constructor(
        ...
        this.handlers = {
            ...
            CUSTOMIZE_PACKAGE_REQUEST: this.handleCustomizePackageRequest.bind(this),
            TALK_TO_EXPERT_REQUEST: this.handleTalkToExpertRequest.bind(this),
    ...
    private async handleTalkToExpertRequest(data: {
        requestId: string;
        name: string;
        phone: string;
        ...
        logger.info(
            `📧 [Email] Sending Talk To Expert request | requestId=${data.requestId}`,
        );

        await this.emailService.sendTalkToExpertAlertEmail({
    ...
    private async handleCustomizePackageRequest(data: {
        requestId: string;
        name: string;
        phone: string;
        email?: string;
        ...
        logger.info(
            `📧 [Email] Sending customize package request email | requestId=${data.requestId}`,
        );

        await this.emailService.sendCustomizePackageAlertEmail({
            requestId: data.requestId,
            name: data.name,
            phone: data.phone,
            email: data.email,
            ...
            eventName: data.eventName,
            customizations: data.customizations?.join(', '),
            createdAt: data.createdAt,
        });
    }
